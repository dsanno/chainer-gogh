...
def save_image(img, width, new_w, new_h, it, layer):
    def to_img(x):
        ...
        Image.fromarray(im).save("{}/im_{}_{}.png".format(args.out_dir, layer, it))
...
def generate_image(img_orig, img_style, width, nw, nh, max_iter, lr, layer, alpha, beta, img_gen=None, lam=1.0):
    ...
    for i in range(1, max_iter + 1):
        ...
        for l in range(len(y)):
            ...
            L1 = np.float32(lam) * np.float32(alpha[l])*F.mean_squared_error(y[l], Variable(mid_orig[l].data))
            L2 = np.float32(beta[l])*F.mean_squared_error(gogh_matrix, Variable(style_mats[l].data))/np.float32(len(y))
            L += L1+L2
        ...
        if i%50==0:
            save_image(img_gen, W, nw, nh, i, layer)
...
parser.add_argument('--orig_img', '-i', default=None,
                    help='Original image')
parser.add_argument('--style_img', '-s', default=None,
                    help='Style image')
...
W = args.width
if args.orig_img != None:
    img, nw, nh = image_resize(args.orig_img, W)
else:
    img, nw, nh = image_resize(args.style_img, W)
...
layer_size = len(nn.alpha);
if args.orig_img != None:
    alpha = np.identity((layer_size), dtype=np.float32)
    beta = np.zeros((layer_size, layer_size), dtype=np.float32)
else:
    alpha = np.zeros((layer_size, layer_size), dtype=np.float32)
    beta = np.identity(layer_size, dtype=np.float32)

for layer in range(layer_size):
    generate_image(img, img, W, nw, nh, img_gen=None, max_iter=args.iter, lr=args.lr, alpha=alpha[layer], beta=beta[layer], layer=layer, lam=1.0)
